...
retweets_per_day <- ggplot(retweet_d8_tweets_by_date, aes(day, retweets_per_day)) +
  geom_line(aes(y = tweets_per_day), color = "orange")+
...
all_graphs <- tweets_per_day / likes_per_day / retweets_per_day + plot_layout(widths = c(3, 1))
...
flattened_df <- unnest(stop_words_tweet_dfs)
...
# Merge with NRC lexicon 

nrc <- get_sentiments("nrc") # get specific sentiment lexicons in a tidy format
random_nrc_words <- random_mergeable_word_count %>%
  left_join(nrc, by="word")
##is inner join the best to use? may be better to do left-Join to calculate actual percentage and ID non-sentiment associated words
...
* Join random sentiment dataframe with delta 8, compare % differences among each 
* 